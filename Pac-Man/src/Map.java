...
public class Map {
	...
	public Map() {
		reset(); // for the sake of simplicity, since repopulating the map and
					// creating it for the first time are essentially the same
					// operation
		// ...
	}
		...
		try {
			...
			while (file.hasNextLine()) {
				...
				case "PacMan":
					...
					break;
				/*
				 * // temporarily commented out Ghost subclass constructors so
				 * the game can be run case "Bashful": object = new Bashful();
				 * break; case "Pokey": object = new Pokey(); break; case
				 * "Shadow": object = new Shadow(); break; case "Speedy": object
				 * = new Speedy(row, column, verbalDirection, this); break;
				 */
				}
	...
	public void reset() {
		// returns all objects to the locations they occupy at the start of the
		// game, and replaces any missing PacDots and PowerPellets
	...
	public int[] getAdjacentLocation(MovableObject object) {
		...
		if (direction == Direction.LEFT) {
			...
			location[1] = y;
			return location;
		} else if (direction == Direction.UP) { // Shouldn't return
												// NullPointerException, if
												// MovableObject ain't on border
												// walls
			location[0] = x;
			location[1] = y - 1;
			return location;
	...
	public Object move(MovableObject object, int x, int y) {
		...
		// need to also remove the object from its previous location to avoid
		// duplication
	}
